...
def main():
    ...
    try:
        ...
        print("Run find_deals_with_api.py first!")
        sys.exit(1)
    
    all_deals = []
    
    # Parse Golf Clearance Warehouse deals
    if raw_data['deals'].get('gcw_shoes'):
        all_deals.extend(parse_gcw_deals(
            raw_data['deals']['gcw_shoes'],
            "https://www.golfclearancewarehouse.com/golf-shoes-s/11.htm"
        ))
    
    if raw_data['deals'].get('gcw_gloves'):
        all_deals.extend(parse_gcw_deals(
            raw_data['deals']['gcw_gloves'],
            "https://www.golfclearancewarehouse.com/category-s/99.htm"
        ))
    
    if raw_data['deals'].get('gcw_clubs'):
        all_deals.extend(parse_gcw_deals(
            raw_data['deals']['gcw_clubs'],
            "https://www.golfclearancewarehouse.com/discount-golf-clubs-canada-s/656.htm"
        ))
    
    # Parse Golf Town deals
    if raw_data['deals'].get('golf_town_clubs'):
        all_deals.extend(parse_golf_town_deals(
            raw_data['deals']['golf_town_clubs'],
            "https://www.golftown.com/en-CA/sale/clearance/clubs/"
        ))
    
    if raw_data['deals'].get('golf_town_shoes'):
        all_deals.extend(parse_golf_town_deals(
            raw_data['deals']['golf_town_shoes'],
            "https://www.golftown.com/en-CA/sale/clearance/shoes/"
        ))
    
    if raw_data['deals'].get('golf_town_gloves'):
        all_deals.extend(parse_golf_town_deals(
            raw_data['deals']['golf_town_gloves'],
            "https://www.golftown.com/en-CA/sale/clearance/golf-gloves/"
        ))
    
    print(f"✅ Parsed {len(all_deals)} deals total")
